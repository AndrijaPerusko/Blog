...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('You have been successfully registered on Blog!')
        return redirect(url_for('login'))
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Your changes has been updated!')
...
def unfollow(username):
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            return redirect(url_for('index'))
        if user == current_user:
            flash('You cant unfollow youself!')
            return redirect(url_for('user', username=username))
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('login'))
    return render_template('reset_password_request.html', form=form, title ='Reset request')
